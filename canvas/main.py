...
class User:
    ...
    def __init__(self, token = os.getenv('MY_CANVAS_TOKEN'), url = "https://uvu.instructure.com/api/v1"):
        self.token = token
        self.url = url
        self.courses = []
        self.assignments = []

    def get_courses(self, enrollment_type: str = "student", enrollment_state: str = "active"):
    ...
    def get_assignments(self, enrollment_type: str = "student", enrollment_state: str = "active"):
        ...
        r = requests.get(
            self.url + "/courses" + f"/{self.courses[0].id}" + '//assignments' + f"?enrollment_type={enrollment_type}&enrollment_state={enrollment_state}",
            headers={"Authorization": f"Bearer {self.token}"})
