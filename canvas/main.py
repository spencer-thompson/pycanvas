...
class Course:
    ...
    def __repr__(self):
        return f"Course({self.friendly_name}, {self.id})"
...
class Assignment:
    def __init__(self, dictionary, course):
        ...
        self.grade_group_students_individually = dictionary['grade_group_students_individually']
        ...
        self.anonymous_instructor_annotations = dictionary['anonymous_instructor_annotations']
        ...
        self.grader_comments_visible_to_graders = dictionary['grader_comments_visible_to_graders']
        self.final_grader_id = dictionary['final_grader_id']
        self.grader_names_visible_to_final_grader = dictionary['grader_names_visible_to_final_grader']
...
class User:
    ...
    def get_assignments(
        self, bucket: str = '',
        enrollment_type: str = "student", enrollment_state: str = "active"):
        """Get assignments from specified course."""
        for course in self.courses:
            ...
            if f'{bucket}' not in self.assignments.keys():
                self.assignments[f'{bucket}'] = []
            for assignment in r.json():
                self.assignments[f'{bucket}'].append(Assignment(assignment, course))
        #return r.json()  # in case you still want to work with the dictionary

def main():
    user = User()
    user.get_courses()
    user.get_assignments(bucket = 'upcoming')
    for course in user.courses:
        print(course)
    for assignment in user.assignments['upcoming']:
